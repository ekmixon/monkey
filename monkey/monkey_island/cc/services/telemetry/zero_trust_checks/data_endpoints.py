...
def check_open_data_endpoints(telemetry_json):
    ...
    events = [
        Event.create_event(
            ...
            message=f'Monkey on {current_monkey.hostname} tried to perform a network scan, the target was {telemetry_json["data"]["machine"]["ip_addr"]}.',
    ...
    for service_name, service_data in list(services.items()):
        events.append(
            Event.create_event(
                ...
                message=f"Scanned service: {service_name}.",
                event_type=zero_trust_consts.EVENT_TYPE_MONKEY_NETWORK,
            )
        )
        ...
        if service_name in HTTP_SERVERS_SERVICES_NAMES:
            ...
            events.append(
                Event.create_event(
                    title="Scan telemetry analysis",
                    message=f'Service {service_data["display_name"]} on {telemetry_json["data"]["machine"]["ip_addr"]} recognized as an open data endpoint! Service details: {json.dumps(service_data)}',
                    event_type=zero_trust_consts.EVENT_TYPE_MONKEY_NETWORK,
                )
            )
